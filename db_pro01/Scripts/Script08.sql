 ...
 */
SELECT EMPLOYEE_ID AS 직원
	  ,FIRST_NAME || ' ' || LAST_NAME AS 이름 -- 이름 합치기
	  ,LOWER(CONCAT(EMAIL, '@example.com'))AS 이메일 -- 이메일 뒤에 주소 붙이기 , 소문자로만 나오게 하기
	  ,REPLACE (PHONE_NUMBER,'.','-') AS 전화번호 -- .을 -로 바꾸기
	  ,TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일"') AS 고용날짜
	  ,FLOOR(SYSDATE - HIRE_DATE)|| '일' AS 근속일
	  ,FLOOR(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)) || '개월' AS 근속월
	  ,FLOOR(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/ 12)|| '년' AS 근속년
	  ,TO_CHAR(TRUNC(SALARY *1260 * (1+ NVL(COMMISSION_PCT,0)), -3),'999,999,999L') AS 급여
	   ,JOB_ID AS 사원ID
	  ,MANAGER_ID AS 매니저ID
 ...
 */
SELECT NVL(SUBSTR(PHONE_NUMBER,1,3), '총계') AS 회선
		,COUNT(*) AS 집계수
FROM EMPLOYEES
