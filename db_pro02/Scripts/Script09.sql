 ...
 *		1. 관리자 계정으로 devAdmin 계정을 새로 만들고 데이터베이스 원격 접속화
 *		   테이블 생성, 수정, 삭제 / 데이터 추가, 수정, 삭제 / 뷰 테이블 생성을 
 *		   할 수 있는 권한을 부여 한다.
 *
 *		2. devAdmin 계정으로 접속하고 회원관리를 위한 테이블(USER_ACCOUNT)을 생성한다.
 *		   테이블에는 ID, 회원계정명, 회원패스워드, 회원이름 정보를 관리 할 수 있도록 컬럼을 생성한다.
...
CREATE USER kkurit IDENTIFIED BY Database1234;

GRANT CONNECT, RESOURCE, CREATE SESSION
    , CREATE VIEW, INSERT ANY TABLE
    , UPDATE ANY TABLE, DELETE ANY TABLE
  TO kkurit;
ALTER USER kkurit quota 10M ON DATA;
 
SELECT * FROM ALL_USERS WHERE USERNAME = 'kkurit';
SELECT * FROM DBA_ROLE_PRIVS WHERE GRANTEE = 'kkurit';
SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'kkurit';
