/*
*	지출내역서(가계부)를 위한 테이블을 만들어본다.
*		- 날짜, 입금액,출금액, 비고를 저장할 수 있는 컬럼이 필요하다.
...
/*
*	위에서 만든 지출 내역서에 다음의 컬럼 정보를 추가 및 수정 한다.
...
ALTER TABLE 지출내역서 ADD ACCOUNT_TYPE NUMBER CONSTRAINT FK_지출내역서_ACOUNT_TYPE REFERENCES 지출내역구분(구분ID);


지출내역서_T
	식별						입금액	  출금액	    비고	    구분			
	 1		2022-04-28		1000         0      저금	 	  2
